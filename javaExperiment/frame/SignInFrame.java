...
import javaExperiment.common.Global;
...
public class SignInFrame extends BaseFrame {
	...
	private JTextField serverIP_textField;
	...
	private void initialize() {
		...
		serverIP_textField = new JTextField();
		serverIP_textField.setText("127.0.0.1");
		serverIP_textField.setBounds(141, 200, 92, 21);
		frameMain.getContentPane().add(serverIP_textField);
		serverIP_textField.setColumns(10);
		
		JLabel serverIP_lable = new JLabel("服务器IP");
		serverIP_lable.setBounds(34, 200, 92, 21);
		frameMain.getContentPane().add(serverIP_lable);
...
//				tip.setVisible(true);
				...
				User user;
				try {
					String ip = serverIP_textField.getText();
					if(ip.length() == 0) {
						JOptionPane.showMessageDialog(frameMain, "服务器IP未设置", "警告", JOptionPane.YES_NO_OPTION);
						return;
					}
					Global.ip = ip;
